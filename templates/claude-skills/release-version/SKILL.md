...
description: 发布版本 / 升版本号 / 出 release / 打 tag 时使用。当用户说"发个版本、出 release、升版本号、打 tag、准备发布、出个 X.Y.Z"时触发。触发即允许创建本地 release 提交与本地 tag，但永不 push。先阻断验证、按 commit 定 SemVer、CHANGELOG 先行，再打 tag。
---
...
可交付物：<本项目的可交付物>（构建产物 / 镜像 / 制品等，以项目实际为准）。
...
- 构建**全部可交付物**并跑通**全部测试**：<构建本项目全部可交付物>。
...
- 注：发快照 / 开发版走 `publish-snapshot` 技能；本技能只出正式 tag 版本。
